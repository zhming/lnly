...
//        }
        ...
        try {
            for(String id : idss){
                if(StringUtils.isNotBlank(id)){
                    LocalCompensationDetail entity  = localCompensationDetailService.selectByPrimaryKey(Long.parseLong(id));
                    if(entity != null ){
        ...
        try {
            for(String id : idss){
                if(StringUtils.isNotBlank(id)){
                    LocalCompensationDetail entity  = localCompensationDetailService.selectByPrimaryKey(Long.parseLong(id));
                    if(entity != null ){
                        localCompensationDetailService.deleteByPrimaryKey(entity.getId());
                    }
                }
        ...
        try {
            ...
            if (resolver.isMultipart(request)) {
                ...
                while (it.hasNext()) {
                    ...
                    //解析数据
                    List<LocalCompensationDetail> list =   processExcelAll(localFile);

                    List<LocalCompensationDetail> insertList =   new ArrayList<>();

                    int index = 0;

                    if(null != list){
                        for  (int i = 0; i < list.size(); i++){
                            LocalCompensationDetail entity = list.get(i);
                            index = i + 1;
                            ...
                            Double areaDb = localCompensationDetailService.findSmallClassData(entity);
                    ...
                    if(errorMap.isEmpty()){
                        localCompensationDetailService.insertList(insertList);
                        resultMap.put("status", 200);
    ...
    private List<LocalCompensationDetail> processExcel(String filePath) throws Exception{
        ...
        try {
            ...
            ExcelWorkSheetHandler<LocalCompensationDetail> workSheetHandler =
                    new ExcelWorkSheetHandler<LocalCompensationDetail>(LocalCompensationDetail.class, cellMapping);
    ...
    private String buildKey(LocalCompensationDetail entity){
    ...
    private List<LocalCompensationDetail> processExcelAll(String filePath) throws Exception{
        List<LocalCompensationDetail> result = new ArrayList<>();
        ...
        if(null != sheets){
            ...
            if(null != rows)  {
                ...
                for(int i = 0; i <length;i++){
                    ...
                    LocalCompensationDetail entity = new LocalCompensationDetail();
